...
wheretoread=open("/Users/apple/Desktop/summerresearch/Apai/Trappist/DataTable2.txt")
for line in wheretoread.readlines():
	string=line.split('\t')
	filenamelist.append(string[3])
	dxlist.append(float(string[4]))
	dylist.append(float(string[5]))
...
def RemoveBadPixels(image,DQ,error,size):
	for i in range(size):
		for j in range(size):
			if DQ[i][j]==4 or DQ[i][j]==16 or DQ[i][j]==512:# or IsCosmicRay(image,error,i,j,size):
...
def CalculateXc():
	...
	#pl.show()
	#StarImage=StarImage[130:170,25:65]
	...
	pl.imshow(StarImage,vmin=0,vmax=100)
	pl.show()
	#xc,yc=fitPeak(StarImage,20,20,10,{'amp': 17000,
     #                    'x0': 10,
      #                   'y0': 10,
       #                  'sigma_x': 1.0,
        #                 'sigma_y': 1.0})
	#print(xc,yc)
	yc,xc=fitPeak(StarImage,143.5,31,15,{'amp': 17000,
                         'x0': 15,
                         'y0': 15,
                         'sigma_x': 1.0,
                         'sigma_y': 1.0})
...
def WavelengthCalibration(xc,yc,dx):
	a00=8954.31
	a01=9.35925e-2
	a10=4.51423e1
	a11=3.17239e-4
...
def flat_field(dx):
	wavelength=WavelengthCalibration(xc,yc,dx)
...
xc,yc=CalculateXc()
print(WavelengthCalibration(xc,yc,0))
...
for i in range(len(filenamelist)):
	...
	dx=int(dxlist[i])
	dy=int(dylist[i])
	wav=WavelengthCalibration(xc,yc,dy)
	frame=flat_field(dx)
	...
	for i in range(5):
		...
		f=RemoveBadPixels(hdulist[5*i+1].data[5:261,5:261],hdulist[5*i+3].data[5:261,5:261],hdulist[5*i+2].data[5:261,5:261],256)
		flatted.append(f)
		flattede.append(fe)
	for i in range(5):
		if i == 4:
	...
	#pl.show()
	totalflux.append(sum(sum(image[130-dx:180-dx,55-dy:210-dy])))
	totalerror.append((np.sqrt(sum(sum(trueerror[130-dx:180-dx,55-dy:210-dy])))))
	time.append(hdulist[0].header['EXPEND'])
	print(sum(sum(image[130-dx:180-dx,55-dy:210-dy])))
	if sum(sum(image[130-dx:180-dx,55-dy:210-dy]))<58000000 or sum(sum(image[130-dx:180-dx,55-dy:210-dy]))>60000000:
		print(name)
	#	pl.imshow(image)
	#	pl.show()
		abnormalfile.write(name)
...
flux=np.array(totalflux)
error=np.array(totalerror)
time=np.array(time)
#index=np.argsort(time)
#newflux=[flux[index[i]] for i in range(len(filenamelist))]
#newerror=[error[index[i]] for i in range(len(filenamelist))]
...
savefile=open("/Users/apple/Desktop/summerresearch/Apai/Trappist/Raw_light_Curve2.txt",'w')
for i in range(len(time)):
	savefile.write(str(time[i]))
	savefile.write('\t')
	savefile.write(str(flux[i]))
	savefile.write('\t')
	savefile.write(str(error[i]))
	savefile.write('\t')
